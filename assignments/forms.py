...
class CreateAssignmentForm(ModelForm):
    class Meta:
        ...
        labels = {
            ...
            'assignment_description': 'Descripción',
            'due_date': 'Fecha de Entrega (yyyy-mm-dd HH:MM)',
            'course': 'Curso'
        }
...
class SubmitAssignmentForm(ModelForm):
    class Meta:
        ...
        fields = ('topic', 'description', 'assignment_file',
                  'assignment_ques', 'author')

    def __init__(self, *args, **kwargs):
        user = kwargs.pop('user')
        assignment = kwargs.pop('assignment_id')
        super().__init__(*args, **kwargs)
        self.fields['assignment_ques'].queryset = self.fields['assignment_ques'].queryset.filter(
            pk=assignment)
        self.fields['author'].queryset = self.fields['author'].queryset.filter(
